...
export function SideReserve() {
  ...
  return (
    <Link
      ...
      className={`dock group hidden xl:flex fixed right-6 2xl:right-12 z-30 items-center px-6 py-4 2xl:px-7 2xl:py-5 rounded-full bg-(--color-base-dark)/85 hover:bg-(--color-base-dark) backdrop-blur-[10px] border border-(--color-base-light)/15 text-(--color-base-light) shadow-[0_12px_36px_-10px_rgba(26,20,16,0.6)] hover:shadow-[0_16px_48px_-10px_rgba(26,20,16,0.7)] hover:scale-[1.025] transition-[background-color,transform,box-shadow] duration-500 ease-[cubic-bezier(0.2,0.8,0.2,1)] ${locale === "ja" ? "top-1/2 -translate-y-1/2 vrl" : "bottom-6 2xl:bottom-12"}`}
    >
      ...
      />
      <span className={`font-mincho text-[clamp(var(--fs-lv4),0.66vw,21.11px)] whitespace-nowrap ${locale === "ja" ? "tracking-[0.4em]" : "tracking-[0.12em]"}`}>
        {SITE.reserveDock}
      </span>
      <span
        aria-hidden
