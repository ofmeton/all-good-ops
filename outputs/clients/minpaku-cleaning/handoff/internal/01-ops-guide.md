...
**本番デプロイ後にここへ実値を追記する余白（Plan 4 Task 6-9 で埋める）:**

```
本番 Supabase Project URL: <デプロイ後に追記>
本番 Vercel URL: <デプロイ後に追記>
LINE Channel ID: <Phase B で追記>
LINE Official Account ID: <Phase B で追記>
Resend From Email: <Phase B で追記>
```
...
> マイグレーションは **追加のみ** とする。既存マイグレーション（0001-0006）の改変は基本しない。スキーマ変更は新規マイグレーションで実現する。
