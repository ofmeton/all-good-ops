...
- アカウント所有:
  - **工藤陸**（Vercel/Supabase/Resend/GitHub）— 将来クライアントへ移管可能
  - **クライアント**（LINE Developer Account + Provider + Channel + Official Account）— LINE Channel は移管不可（[公式仕様](https://developers.line.biz/en/docs/messaging-api/) 確認済み）のため最初からクライアント名義。工藤は Official Account Manager に管理者として参加して運用代行
- ランニングコスト: 当面工藤陸負担（設計書 §1 の友人負担前提とは一旦不一致だが、移管後は変更可）。LINE は最初からクライアント名義だが Free 枠内のため発生なし
...
**実行者の分担（C 案準拠）:**
- **LINE 開設はクライアント側で実施**（理由: LINE Channel は別 Provider/別アカウントへ移動不可。"Once you create a channel, you can't move the channel to another provider later." — [LINE Developers Docs](https://developers.line.biz/en/docs/messaging-api/) で公式仕様確認済み。将来 ownership 移管時に Channel ごと譲渡できないため、最初からクライアント名義で開設する）
- **Resend は工藤側で実施**（Free プランの送信元 `onboarding@resend.dev` で当面運用。Domain 取得不要。移管時はクライアントが新規アカウント作成 → API キー再発行で対応）

- [ ] **Step 1: クライアントへ LINE 開設依頼文を作成・送付**

controller が `outputs/clients/minpaku-cleaning/handoff/internal/03-line-setup-request-draft.md` にクライアント向け依頼文をドラフト。ユーザー（工藤）が文面を確認後、別経路（メール/Slack 等）でクライアントへ送信。

依頼文に含める内容:
- 背景: LINE Channel は移管不可な仕様のため、最初からクライアント名義で開設してほしい旨
- 開設手順（手順1〜6）:
  1. https://developers.line.biz/console/ に LINE アカウントでログイン（クライアント個人 LINE で可）
  2. Provider 作成（名前: 業者名 or `minpaku-cleaning` 等任意）
  3. Provider 内に Messaging API Channel 作成
     - Channel name: `民泊清掃管理アプリ通知`
     - Category: ライフスタイル
     - Subcategory: 適切なもの
  4. Channel Basic settings タブで Channel access token (long-lived) を発行
  5. LINE Official Account Manager で「あいさつメッセージ」「応答メッセージ」をオフ（通知のみで使用するため）
  6. **工藤陸（off.me.ton@gmail.com）を Official Account Manager の管理者として追加**（運用代行のため）
- 工藤へ共有してほしい情報:
  - Channel ID
  - Channel Access Token（チャットには貼らず、後で Vercel 環境変数登録時にクライアントが直接入力するか、安全な経路で受領）
  - 友だち追加用 QR コード/URL（スタッフ配布用）
...
4. API キーを copy（一度きり表示。Vercel 環境変数登録時まで安全に保管）

- [ ] **Step 4: クライアントからの LINE 情報受領待ち**

クライアントから Channel Access Token + 友だち追加 QR/URL が届いたら、内容を確認して OPS_GUIDE.md に Channel ID（トークン本体は除く）を追記。Vercel 環境変数登録 (Task 8) へ進む。

- [ ] **Step 5: 取得した値を OPS_GUIDE に追記（キー本体は除く）**

```bash
cd outputs/clients/minpaku-cleaning
git add handoff/internal/01-ops-guide.md handoff/internal/03-line-setup-request-draft.md
git commit -m "docs(handoff): OPS_GUIDE に LINE/Resend 情報を追記 + LINE 開設依頼文ドラフト"
```
