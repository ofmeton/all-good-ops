...
| パス | 責務 |
|---|---|
| `supabase/migrations/0003_ad_ops.sql` | articles enum 拡張 + 列追加、sources boolean 3列化、インデックス |
...
**Files:**
- Create: `supabase/migrations/0003_ad_ops.sql`

- [ ] **Step 1: マイグレーション SQL 作成**

ファイル `supabase/migrations/0003_ad_ops.sql` を以下内容で作成：
...
適用後 `mcp__plugin_supabase_supabase__list_migrations` で `0003_ad_ops` が登録されていることを確認。さらに `mcp__plugin_supabase_supabase__execute_sql` で：
...
git add supabase/migrations/0003_ad_ops.sql
...
Supabase 本番に `0003_ad_ops.sql` が適用されているかを確認：
