...
既存 `articles` テーブルに列追加（別テーブル化はしない。理由：多重所属容易、JOIN回避、既存スコア列と並列で自然）。
...
-- 多重所属フラグ
ALTER TABLE articles ADD COLUMN ad_ops_in_pipeline       boolean DEFAULT false;
```

`sources` テーブルは既存 `pipeline` カラムを廃止し、`in_opportunity` / `in_business_defense` / `in_ad_ops` の boolean 3列化する。理由：多重所属ソース（例：PR Times AI が機会発見と事業防衛の両方に属する既存ケース）を正しく表現でき、今後の Phase 追加（例: `in_kodomo_ibasho` 等）でも拡張容易。
...
| DBスキーマ | 既存 `articles` テーブルに列追加（別テーブル化しない） |
| Sources 多重所属 | `in_opportunity` / `in_business_defense` / `in_ad_ops` の boolean 3列化 |
