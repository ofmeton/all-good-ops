...
| タスク | プラグインスキル |
|---|---|
| 機能実装・設計 | `superpowers:brainstorming` → `writing-plans` → `subagent-driven-development`（既定）。まとまった機能で**仕様照合(spec-validator)や工程分離の人間CP3点を重視**する時は `feature-factory`。両者は重複し brainstorm→plan→subagent-driven が等価カバー＝既定、feature-factory は照合重視時に明示選択 |
