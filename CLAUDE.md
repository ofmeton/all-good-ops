...
- **動画生成環境**: `outputs/publishing/remotion/`（Remotion 4 + React 19 + Tailwind v4 + TS）。Claude が Composition.tsx を書き換えて X 動画 / IG リール / note 内動画を生成。SSOT は同ディレクトリ README。担当: visual-designer
...
- **ofmeton**: 個人ブランド発信（X / Instagram / note）+ 進行中個人案件。主軸。担当: brand-publisher / client-manager
- **工藤陸**（本名）: 既存契約済 個人案件（terra-isshiki / minpaku-cleaning）の請求・契約のみ。担当: client-manager
- **はぐりん**（persona）: 収益化コンテンツ。`monetize-os/growth-lead` に委譲（このリポでは扱わない）
...
2. 現ブランチを判定し、以下のルールに従って必要なら新 task ブランチを切る:

   | 現ブランチ | 新依頼の主題 | アクション |
   |---|---|---|
   | `main` / `master` / `improve/iteration-*`（保護） | 任意 | **必ず新 task ブランチを切る** |
   | `task/*` | 現ブランチ名と**一致 or 関連** | 現ブランチで継続 |
   | `task/*` | 現ブランチ名と**別主題** | **編集着手前に新 task ブランチを切る**（前主題 commit との混在防止） |
...
3. **判断結果を 1 行で明示**してから作業着手:
   - 例: `🌿 現ブランチ task/260521-raw-facts-rule で継続（主題一致）`
   - 例: `🌿 task/260520-article-backlog → task/260521-raw-facts-rule に切替（別主題）`
   - 「branch を切るかどうか」を黙って判断しない。ユーザーが事後に整理整頓を強いられる事故を防ぐ
4. 秘書 / メインセッション直接対話 ともにこの Step 0 を実行する

**照会のみ（編集なし）の依頼**:
- 「進捗確認」「状態確認」「ファイル参照のみ」の場合はブランチ判定不要
...
**並列セッション時は worktree で物理隔離**（2026-05-21 追加）:

ブランチで分離しても **git cwd の HEAD は 1 つしかない**ため、別 Claude セッションが同じリポジトリで `git checkout` するとこちらの作業中 HEAD が書き換わる事故が起きる（2026-05-21 で発覚）。並列セッション運用時は `git worktree` で作業ツリーごと分離する。

```bash
# 並列セッション B 用に worktree を切る
git worktree add ../all-good-ops-<topic> task/$(date +%y%m%d)-<topic>
cd ../all-good-ops-<topic>
# このセッションはこの cwd で完結。元 cwd の HEAD には触らない
```
