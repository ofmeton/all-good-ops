...
- **ポジション**: 「エンジニアだけど、難しいことを誰でも伝わる平易な言葉で噛み砕いて届ける実装者」。ターゲット = 基盤は AI を仕事に活かしたいビジネスパーソン全般を見据えつつ、**メイン = Claude Code を既にある程度使っていて、有益な活用法を探している層**
- **名義境界（行動制約）**:
...
## knowledge（~/vault セカンドブレイン）
- 着手前に ~/vault/INDEX.md を入口に、関連する entities/ concepts/ を読む（フルスキャン禁止・INDEX からリンクを辿る）
- 事業・顧客・オーディエンスに関する主張は必ず vault の該当ページ（出典付き）に接地する
