    ...
    <div className="px-4 sm:px-5 pt-3">
      <div className="rounded-lg border border-amber-200 bg-amber-50/60 p-3">
        <div className="flex items-baseline justify-between gap-2 flex-wrap">
          <span className="text-xs font-medium text-amber-800">元ネタのメディアを添付</span>
          ...
          {withMedia.map((s) => {
            ...
            return (
              <div key={s.id} className="space-y-2">
                {s.source_ref && (
                  <p className="text-[11px] text-slate-500">@{s.source_ref}</p>
                )}
                    ...
                    if (isVideoType(m.type)) {
                      return (
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 rounded-lg border border-blue-300 bg-white px-2.5 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                        >
                      ...
                      >
                        ...
                        <span
                          className={`absolute bottom-0 inset-x-0 text-center text-[10px] font-bold py-0.5 ${
                            on ? "bg-emerald-600 text-white" : "bg-black/45 text-white"
                          }`}
                        >
                          {on ? "✓ 添付中" : "添付する"}
                        </span>
        ...
        <p className="mt-2.5 text-[11px] leading-relaxed text-amber-700/90">
