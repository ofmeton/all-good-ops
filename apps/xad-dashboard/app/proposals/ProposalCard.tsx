...
const RANK_STYLE: Record<string, string> = {
  A: "bg-emerald-400/15 text-emerald-300",
  B: "bg-blue-400/15 text-blue-300",
  C: "bg-white/5 text-slate-300",
};
...
export function ProposalCard({
  ...
  const rankStyle = proposal.rank ? (RANK_STYLE[proposal.rank] ?? "bg-white/5 text-slate-300") : "bg-white/5 text-slate-400";
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-surface shadow-sm overflow-hidden">
      ...
      {/* hypothesis */}
      <p className="px-4 sm:px-5 pb-3 text-sm text-slate-200 leading-relaxed">
        {proposal.hypothesis}
      </p>
      ...
      <details className="px-4 sm:px-5 pb-3 group">
        ...
        <pre className="mt-2 text-[11px] bg-white/[0.03] border border-white/5 rounded-lg p-3 overflow-x-auto text-slate-300 leading-relaxed">
      ...
      <details className="px-4 sm:px-5 pb-3 group">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <select
            ...
            className="text-xs rounded border border-white/10 px-2 py-1.5 text-slate-300 focus:outline-none focus:ring-1 focus:ring-blue-400/30"
          >
          ...
          <input
            ...
            className="text-xs rounded border border-white/10 px-2 py-1.5 w-28 text-slate-300 focus:outline-none focus:ring-1 focus:ring-blue-400/30"
          />
        </div>
        {apply && (
      ...
      {/* actions */}
      <div className="px-4 sm:px-5 py-3 bg-white/[0.03] border-t border-white/5 space-y-2">
        <div>
