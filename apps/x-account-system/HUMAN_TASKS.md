...
- [x] `wrangler.toml` (cron schedule) + `src/worker.ts` scaffold 作成済 → 反映手順は `DEPLOY.md` (`wrangler login` → secret put → deploy)
...
- [ ] cron 実行基盤を選定 → Python のため Workers 不可。**GitHub Actions 推奨** (`DEPLOY.md` 「Python ジョブは Workers 対象外」節)。UTC `0 22 * * *` = 07:00 JST
