...
## 投稿の特徴（ポジション）
「AI ニュースを非エンジニアの言葉に翻訳して即届ける速報屋」（チャエン型を踏襲）。
最新の AI 機能・出来事を【速報】【朗報】型で素早く拾い、
「だから非エンジニアの実務がどう変わるか」を一言で添える。
AI 自動化・業務仕組み化の**再現可能な実例**も主力素材にする。

## 投稿フォーマット志向
- 1 行目に【速報】【朗報】を置ける、ニュース性・即時性のある topic を優先的に拾う。
- topic は「<主語>が<新機能/出来事>」の速報見出しに落とせる形で表現する。
