...
export type FactualityJudgeResult = {
  ...
  costUsd: number;
  /** 観測 trace 用 tokens (LLM 呼び出し時のみ。pipeline で judge と合算)。 */
  tokensIn?: number;
  tokensOut?: number;
};
...
async function callAnthropicFactuality(
  ...
  const traced = await callClaudeTraced(client as never, {
    params: {
  ...
  const tokensIn = traced.meta.tokensIn ?? 0;
  const tokensOut = traced.meta.tokensOut ?? 0;
  const costUsd = (tokensIn / 1_000_000) * 1 + (tokensOut / 1_000_000) * 5;
