...
- タイトル: 「【完全保存版】/【2026最新】/【保存版】 + ツール名 + 読者が欲しい到達状態 + 全手順/完全ガイド/N選」。**短く強く1つに絞る**。保存価値・具体数字・到達状態のうち2つ以上を入れるが、括弧注釈（「（公式準拠の全手順）」等）を後ろに足して長くしない。数字や行動動詞で「自分はまだできていない」と思わせる。
- preview冒頭（最重要・記事の離脱はここで決まる。3段で組む）: ①読者の悩みを「」で1〜2つ代読する → ②**自分の1人称の失敗体験を1〜2文で自己開示する**（「僕も最初は〜していた」と具体的な情景で。理屈でなく実体験）→ ③「最近わかってきたのは〜」と情報格差を提示する（できれば数字を添える。例「混同するとコストが3倍になる」）→ ④**preview末尾は読了の見返り＝「読むと何ができるようになるか（到達状態）」を1文**で示して読了動機を確定させる。**「先に結論を置きます」「整理します」「本記事では」「結論から言うと」、および「今日は〜整理します／全部置いていきます」のような“これから何をするか”の告知で preview を締めない**（AI感・冗長で離脱する）。結論や見返りは宣言調でなく、自己開示と情報格差の流れの中で自然に滲ませる。
- 本文 skeleton:
  1. 導入: 悩み代読 →【1人称の自己開示＝失敗体験】→ 情報格差の提示（preview冒頭の流れをそのまま受ける。宣言調にしない）
  2. 全体像: 設定/設計/手順を変えると何が楽になるかを、宣言でなく具体的な場面・before/after で示す
  ...
  11. 参考（公式一次ソース）: 本文で根拠にした公式ドキュメント/発表の**名称を3〜4個列挙**する（「📎 参考」見出し）。URLは元ネタ source に在るもの or 確実なものだけ。無ければドキュメント名だけにし、URLは捏造しない。
  12. CTA: オープンチャット/相談/関連記事/引用感想のいずれかを1つ
- 見出しは 7〜10 本を目安に番号付きで置く。各章は「結論 → 手順/具体 → 注意点」の順。本文は X Article 用なので、番号見出しやチェックリストを使う。
- 読み手を動かす設計（保存版の核・必須）: ①**実務系の章の末尾に必ず「▶ 今日できる1アクション:」を1行**置き、読んで終わりにさせない（ロールモデルの最大の勝ち筋）。②**本文中に最低2箇所、コピペ用プロンプト**を「（コピペ用）〜」の形で具体的に置く。③**実行可能なコマンド例**（例 \`claude --worktree <name>\`）を入れる。④まとめ直前に「今日から試す3ステップ」を□チェックボックスで置く。これらが無いと D4（実行可能性）が大きく落ちる。
- 保存される理由を本文内に作る。コマンド、設定パス、プロンプト、チェックリスト、FAQ、参考リンクのうち複数を入れる。
- root投稿側で説明し切らない。Article card のタイトル/preview が働く前提で、本文内に保存価値を集約する。
- 避けること: タイトルだけ強く本文が薄い / 悩み引用なしでいきなり仕様説明 / 落とし穴なしの楽観論 / 事実確認できない数字の盛り / **「先に結論を置きます」等の機械的な前置き / 1人称の体験・判断がゼロの説明書き（各章にも体感を1つ入れる）/ 章の繋ぎをテンプレ化する**。
- 人間っぽさ（最重要）: 全体を通して**書き手の1人称の体験・判断・手触り**を滲ませる（preview の自己開示だけで終わらせず、各章にも「実際やってみると〜」級の実感を1つ入れる）。リズムは一定にせず文の長短を混ぜる。

## 文体・密度・フックの手本（この質感を再現する。**内容は必ず今回の元ネタに置き換える**。文章をそのまま流用しない）
トップ保存版記事の冒頭はこの質感:
---
...
最近わかってきたのは、1人で複数の Claude Code を並行で回す仕組みを Claude Code 自身が公式に3つ用意しているということです。しかも似て非なるもので、混同すると逆に事故ります。役割で分けてしまえば、自分はコードを1行も書かず「指揮する側」へ回れます。

今日はその3つの仕組みと、役割境界の正しい固定方法を、公式ドキュメントの一次情報で確認しながら整理します。
---
手本から盗む点: ①悩みの「」代読が具体的で痛い ②「僕も最初は〜眺めていました」の情景つき自己開示 ③「最近わかってきたのは〜」の情報格差 ④各章末「今日できる1アクション」と誤解訂正(よくある説明は実は公式と違う)で独自価値 ⑤敬体で淡々・でも体験の手触りがある。この**密度とフックの作り方だけ**を真似て、内容は今回の元ネタで書く。

各章はこの密度・締め方を手本にする（誤解訂正＋末尾アクションの例。内容は元ネタで置換）:
---
4. 役割の固定は「CLAUDE.md」ではなく「subagent定義」で

よく「CLAUDE.md に役割IDを書いて起動のたびに切り替える」と説明されますが、これは公式機能として存在しません。公式は「CLAUDE.md は context であって強制される設定ではない」と明記しています。必ず止めたいことには hooks を使う。

役割を固定したいなら .claude/agents/ に name / description / tools / model を書いたファイルを置きます。これは agent teams の teammate としても名前で呼び出せます。

（コピペ用）「.claude/agents/ に置く subagent 定義を1つ作って。役割は『実装のN+1クエリを指摘するレビュアー』。frontmatter に name/description/tools/model を入れて」

▶ 今日できる1アクション: .claude/agents/ に reviewer 役を1つ定義して、直近のコードをレビューさせる。
---
手本から盗む点: 「よくある説明は実は公式と違う」の誤解訂正で独自価値を出す／一次ソースで裏取り／コピペ用プロンプトを実際に置く／章末に「▶ 今日できる1アクション:」で必ず動かす。各実務章をこの密度で。

## 参考構造（章立ての型）
破綻理由 → 仕組みの整理 → 実践配置 → 誤解訂正×1-2(公式一次ソース) → 落とし穴3点 → チェックリスト → CTA。`,
  },
