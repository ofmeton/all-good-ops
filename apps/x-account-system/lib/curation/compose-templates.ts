...
export const COMPOSE_TEMPLATES: Record<string, ComposeTemplate> = {
  template_chaen_gold: {
    ...
    name: "チャエン型1（黄金）",
    description:
      "速報フック→意味づけ→箇条書き要点→実務接続。最新AIの実用的な使い方を、誰でも伝わる平易な言葉で噛み砕く黄金型。",
...
- **プレーンテキストで書く**（markdown 記法は使わない。強調は言葉と改行で）。`,
  ...
  template_case_calm: {
    ...
    name: "事例咀嚼・敬体型（保存版）",
    description:
      "【保存版】+事例/出典→敬体で冷静に噛み砕く→要点3→一般化→「保存して試すといい」。Claude Code を使う実務者層に届く信頼型。intent=信頼。",
    tone:
    ...
    preferredFmats: ["medium", "long"],
    systemPromptPatch: `## 投稿の型（事例咀嚼・敬体型 / 保存版）
- **敬体（ですます）で統一**する（常体を混ぜない）。これがこの型の最大の肝。
- 1行目: 【保存版】【最新】等 + 事例/出典に基づく事実を 1〜2 行（「〜が話題」「〜という事例が話題になっている」）。
- 前置きの「〜について解説します/整理します」は使わず、すぐ要点を「▶︎」or「・」で 3 点前後に置く（markdown のリスト記号 - * は使わない）。
