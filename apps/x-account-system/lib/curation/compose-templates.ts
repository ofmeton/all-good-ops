...
2. 「【事件】Anthropicが年収\$750,000以上で雇うLLMアーキテクトの知識を、Stanfordが1時間の講義で全部教えてしまった😳しかも完全無料。 ・LLMの設計思想がゼロから ・実装レベルのアーキ解説 …これは講義じゃない。一次情報の解放です🤐」（like 2,040）`,
  ...
  template_reaction_light: {
    ...
    hookType: "共感",
    hookStrength: "strong",
    referenceNote: "6アカ分析 @MakeAI_CEO（短文・拡散特化・thread平均6.7連結）catalog §4 T4",
...
- fmat=thread を選んだ時は「番号付きスレッド（【1】【2】…）で深掘り」する2段構え（短い単発フック → 連結投稿）。本文は区切り「---」で分割すると後段の配管が連結投稿として処理する。
...
- fmat は medium の単発が基本。手順が長い場合は番号スレッド（見出しツイートで止め → 各手順を「---」区切りで連結）。
