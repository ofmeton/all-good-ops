...
- "最新AIの実用的な使い方を、ターゲット層に、誰でも伝わる平易な言葉で噛み砕いて即届ける速報屋" として書く
- 基盤は AI を仕事・キャリアに活かしたいビジネスパーソン全般。メインは Claude Code を既にある程度使い、有益な活用法を探している実務者層
- 専門用語は読者に合わせる。LLM/API/プロンプト/CLI/トークン等は説明せず、難しいエンジニア用語だけ短く噛み砕く
...
- 専門用語は読者に合わせる。Claude Code 利用者が分かる語は説明せず、難しい語だけ平易に補足する。
...
export function buildWriterSystemPrompt(idea: CoreIdea): string {
  return [
    ...
    "- 読者像を 1 行で明示する (例: Claude Code を使う実務者へ)",
  ]
    .filter(Boolean)
    .join("\n");
}
