...
describe("runCollect", () => {
  ...
  test("fail-open: dispatchTool throwing does not kill the run", async () => {
    // explore turn: two tool_uses — get_trends (throws) + search_tweets (returns 1 tweet)
    let turn = 0;
    const fakeAnthropic = {
      messages: {
        create: async (args: { tools?: { name: string }[] }) => {
              ...
              content: [
                {
                  ...
                  input: {
                    scores: [
                      { id: "tweet-good", freshness: 60, velocity: 60, target_fit: 70, overall: 63, reason: "ok" },
                    ],
                  },
                },
              ],
          ...
          if (turn === 1) {
            return {
              stop_reason: "tool_use",
              content: [
                { type: "tool_use", id: "tu-trends", name: "get_trends", input: { woeid: 1 } },
                { type: "tool_use", id: "tu-search", name: "search_tweets", input: { query: "AI", queryType: "Latest", via: "keyword" } },
              ],
              usage: { input_tokens: 20, output_tokens: 10 },
            };
          }
    ...
    const inserted = await runCollect({
      ...
      api: {
        searchTweets: async () => [
          { id: "tweet-good", text: "AI is great", author: { userName: "b" }, createdAt: new Date().toISOString(), likeCount: 200, viewCount: 2000 },
        ],
