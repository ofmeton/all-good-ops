...
-- ---------------------------------------------------------------------------
ALTER TABLE xad.posted_records
  ADD COLUMN IF NOT EXISTS utm_source text,
  ADD COLUMN IF NOT EXISTS utm_medium text,
  ADD COLUMN IF NOT EXISTS utm_campaign text;

CREATE INDEX IF NOT EXISTS idx_posted_utm ON xad.posted_records(utm_campaign, posted_at);

-- 売上 attribution テーブル
CREATE TABLE IF NOT EXISTS xad.business_outcomes (
  id uuid PRIMARY KEY DEFAULT uuid_generate_v4(),
  created_at timestamptz NOT NULL DEFAULT now(),
  trace_id uuid,

  source_post_id uuid REFERENCES xad.posted_records(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_outcomes_type ON xad.business_outcomes(outcome_type, outcome_at);
CREATE INDEX IF NOT EXISTS idx_outcomes_post ON xad.business_outcomes(source_post_id);

ALTER TABLE xad.business_outcomes ENABLE ROW LEVEL SECURITY;
CREATE POLICY outcomes_human_all ON xad.business_outcomes
  FOR ALL TO human_admin USING (true) WITH CHECK (true);
CREATE POLICY outcomes_publisher_rw ON xad.business_outcomes
  FOR ALL TO publisher_agent USING (true) WITH CHECK (true);
CREATE POLICY outcomes_agents_ro ON xad.business_outcomes
...
CREATE INDEX IF NOT EXISTS idx_drafts_bizlaw ON xad.post_drafts(business_law_risk_flag)
...
-- C-13: verified_failure_story 上限化 (materials_store 拡張)
-- ---------------------------------------------------------------------------
ALTER TABLE xad.materials_store
...
CREATE INDEX IF NOT EXISTS idx_materials_verified_failure ON xad.materials_store(verified_failure_story)
  WHERE verified_failure_story = true;

-- failure_story 月 4 本上限を検知する view
CREATE OR REPLACE VIEW xad.verified_failure_supply_monthly AS
  SELECT
...
CREATE INDEX IF NOT EXISTS idx_gates_platform ON xad.launch_gates(platform, passed);

ALTER TABLE xad.launch_gates ENABLE ROW LEVEL SECURITY;
CREATE POLICY gates_human_all ON xad.launch_gates
  FOR ALL TO human_admin USING (true) WITH CHECK (true);

-- ---------------------------------------------------------------------------
-- 完了通知 (Supabase Studio で apply 後の SELECT で確認可能)
-- ---------------------------------------------------------------------------
COMMENT ON TABLE xad.business_outcomes IS 'v10.3 C-10: PCR から売上 attribution';
COMMENT ON TABLE xad.launch_gates IS 'v10.3 C-7: IG / X / note 等の独立 launch gate 監査';
