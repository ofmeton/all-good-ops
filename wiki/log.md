...
## [2026-05-22] phase | 名義3ライン分離 規約撤廃

- `wiki/SCHEMA.md` §名義3ライン分離 セクション削除、frontmatter から `identity:` フィールド削除、lint チェックから「名義3ライン混在」削除
- wiki/ 全 37 ファイルから `identity:` 行を一括削除（sed バッチ）
- `CLAUDE.md` §名義3ラインの切り分け（混在厳禁）削除 + wiki 運用§名義3ライン分離 削除 + 関連文言を「事実関係」表現に緩和
- 関連エージェント/スキル更新: `brand-publisher.md` / `claude-md-health-check.md` / `publishing-wiki-ingest.md`
- `wiki/external/monetize-os-pointer.md` の SCHEMA 参照を直接表現に置換

理由: BSA（工藤陸名義）撤退済（2026-05-20）+ はぐりんは monetize-os 側に物理隔離済の状態で、ガチガチな分離規約の運用負荷が効果を上回るため。役割上の使い分けは事実関係として残存。

参照: raw/facts/situations/2026-05-22-meigi-3-line-abolish.md

## [2026-05-22] phase | claude-obsidian 部分採用（hot.md / Query Modes / manifest / defuddle）

claude-obsidian（Karpathy LLM Wiki 実装、5.3k★）との差分調査結果を踏まえ、4 機能を採用。

採用機能:
- `wiki/hot.md` — ~500 words のホットキャッシュ。セッション開始時に最優先で読む
- query プロトコル拡張: 3 Modes（quick / standard / deep）で token 予算明示化
- `raw/.manifest.json` — md5 ハッシュベース delta tracking。二重 ingest 防止
- `defuddle` CLI — URL ingest 時の HTML クリーニング（任意、未インストール時は WebFetch fallback）

却下: DragonScale 3 機構 (M2/M3/M4) / /canvas / /autoresearch / MCP-Obsidian 直接操作 / ファイル名 Title Case 移行 — ours の規模・既存運用と衝突

実施内容:
- spec 起票: docs/superpowers/specs/2026-05-22-llm-wiki-claude-obsidian-adoption-design.md
- wiki/SCHEMA.md に §3層構造 manifest 例外 / §ingest manifest check 手順 / §query 3 Modes / §ホットキャッシュ を追記
- wiki/hot.md / raw/.manifest.json を新規配置
- wiki/index.md に hot.md 言及追加
- CLAUDE.md「セッション開始時の秘書の動作」§1 に hot.md 最優先読み込みを追加
- publishing-wiki-ingest skill に manifest hash check + hot.md 更新ステップを組込み
- session-retrospective skill に hot.md 更新を Step 7 として追加
- defuddle-usage skill を新設
- scripts/manifest-bootstrap.sh で publishing/inspirations 5 件を manifest にバックフィル

## [2026-05-27] phase | ai-radar 完全撤廃

ai-radar プロジェクトを完全撤廃。Free tier 枠空け含む整理。

実施:
- LaunchAgents 2 plist (`jp.ofmeton.ai-radar-codex` / `jp.ofmeton.ai-radar-export-to-raw`) unload + 削除
- Supabase project 即時削除（Free tier 枠 1 つ空き）
- 外部リポ `/Users/rikukudo/Projects/ai-radar/` を GitHub archive 化 + ローカル削除
- CLAUDE.md: 外部スポーク・横断チーム表・ルーティング表・MCP 説明から ai-radar を除去
- `.claude/agents/ai-radar.md` 削除
- wiki 関連ページを archived マーカー化（ai-radar-pointer.md, claude-usage/index.md, streams.md, index.md）
- scripts/monthly-audit.sh の spoke health check から ai-radar 行削除

保存（履歴・将来の発信転用素材として残置）:
- `outputs/documents/ai-radar/` (10 ファイル: 実装計画 / v2.1 pivot plan / Phase 1 apply guide 等)
- `raw/ai-radar/` (101 ファイル: 素材・hilights)
- `wiki/domain/ai-industry/ai-radar-pointer.md` (archived ステータス)
...
## [2026-06-06] phase | agent-teams 体制 Phase 0 - 設計 SSOT 集約
