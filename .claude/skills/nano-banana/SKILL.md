...
- **source**: https://github.com/kingbootoshi/nano-banana-2-skill （default branch `main`、`plugins/nano-banana/skills/nano-banana/SKILL.md`）
- **取り込み日**: 2026-06-19
- **このリポでの位置づけ**: 既存の visual-design-system / visual-designer は Codex MCP の **gpt-image-2（OpenAI 系）** を使う。nano-banana は **Gemini 系（Gemini 3.1 Flash / 3 Pro）の別エンジン選択肢** として補完。透過アセット（緑背景キーイング）・厳密寸法制御・コスト追跡が欲しい時の代替手段。
- **改変点**: 本文は原文そのまま。この「取り込みメモ」セクションのみ追記。CLI ソース一式（src/cli.ts 等）は取り込んでいない（SKILL.md のみ）。metadata.json を別途作成。
- **実働の前提（未設定時は使用不可）**: npm の `@google/genai` に依存する CLI 込みのため、別途 `npm i @google/genai` と **Google AI Studio の API キー（`GEMINI_API_KEY` 等）設定** が必要。キー未設定では動かない。`/init`（上記手順）でリポ clone + bun install + キー投入まで行う。
- **コスト開示（external-api-cost-disclosure 規約準拠）**: 従量課金 API。**Gemini 3.1 Flash Image: 約 $0.045〜0.151/枚**（512〜4K）、**Gemini 3 Pro: その約2倍**（~$0.134〜0.302/枚）。生成のたびに `~/.nano-banana/costs.json` へ記録され `--costs` で確認可。無料枠は **Google AI Studio アカウント作成**（https://aistudio.google.com/apikey ）で取得可。プラン提示と同じターンに円/回換算コストを併記すること。
