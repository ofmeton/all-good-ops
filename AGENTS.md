...
人間向けの全体方針は `CLAUDE.md`、設計作法の正本は `~/brain/2-atoms/standards.md`。本ファイルは Codex 実装に必須の最小集合のみ。

## 着手前に必ず読む
- **設計 SSOT**: `~/brain/2-atoms/standards.md` の **A 章（スタック非依存の設計規律）は常に適用**。採用スタックがあれば **B 章**の該当節（現状 Next.js+Supabase）も読む。
- ブループリント（Claude architect から渡される）= 実装対象の真実。ファイル一覧・データ契約・API 形・テスト要件はそこに従う。規約に反する選択をする場合は理由をサマリに明記（暗黙の逸脱禁止）。
- **最小実装を旨とする**。投機的機能・単発用途の抽象化・頼まれていない一般化を作らない。BP にない複雑さを足す場合は理由をサマリに明記。
