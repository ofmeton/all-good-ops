...
test("insertTrace は insert が reject しても握りつぶす", async () => {
  __setTraceSupabaseForTest({
    schema: () => ({ from: () => ({ insert: async () => ({ error: { message: "boom" } }) }) }),
  } as never);
  await expect(
    insertTrace({ runId: "r1", stageId: "writer", status: "ok", startedAt: new Date() }),
  ).resolves.toBeUndefined();
...
export async function insertRun(run: RunRow): Promise<void> {
  try {
    ...
    if (!sb) return;
    await sb.schema("xad").from("run").insert({
...
export async function updateRun(
  ...
  try {
    ...
    await sb.schema("xad").from("run").update(row).eq("id", id);
  } catch { /* fail-open */ }
}
...
export async function insertTrace(t: TraceRow): Promise<void> {
  try {
    const sb = getTraceSupabase();
    if (!sb) return;
    await sb.schema("xad").from("run_trace").insert({
...
- [ ] **Step 1: 既存 redact API を確認**

Read `apps/x-account-system/lib/dlp/redact.ts` の export を確認し、文字列を redact する関数名（例 `redact(text)` または `redactText`）を特定する。本タスクのコードはその実関数名に合わせること（以下は `redactText` と仮定。違えば置換）。
...
import { redactText } from "../dlp/redact.js"; // ← Step 1 で確認した実名に合わせる
...
- [ ] **Step 1: 既存 Anthropic 呼び出しの形を確認**

Read `apps/x-account-system/lib/ideation/ideate.ts` の Anthropic 呼び出し（`messages.create` の引数と client 生成）を確認し、本タスクのヘルパが同じ client/SDK を使うよう合わせる。
...
test("text と usage(prompt/tokens) を返す", async () => {
  const fakeClient = {
  ...
  const out = await callClaudeTraced(fakeClient as never, {
    model: "claude-haiku-4-5", system: "sys", prompt: "user-prompt", maxTokens: 100,
  });
...
): Promise<{ text: string; meta: TraceMeta }> {
  const res = await client.messages.create({
    model: args.model, max_tokens: args.maxTokens,
    system: args.system, messages: [{ role: "user", content: args.prompt }],
  });
  const text = res.content.filter((c) => c.type === "text").map((c) => c.text ?? "").join("");
  return {
    text,
    meta: {
      promptText: (args.system ? args.system + "\n\n" : "") + args.prompt,
      model: args.model,
...
export async function handleJob(
  ...
  retry: { attempt: number; maxAttempts: number } = { attempt: 1, maxAttempts: MAX_ATTEMPTS },
): Promise<void> {
  const runId = (msg as { runId?: string }).runId;
  // brownout skip 記録（allowedJobs 外で return する既存分岐の直前/該当箇所に挿入）
  // 既存の brownout 判定で skip する場合:
  //   if (runId) await updateRun(runId, { status: "skipped", finished: true });
  //   if (runId) await recordSkip(ctx, { runId, stageId: "safety", outcome: "brownout" });
  //   return;
  // …既存 switch(msg.job) 本体…
}
```

> 既存 `handleJob` 本体（switch）はそのまま残し、署名 3-4 引数追加 + brownout 分岐に上記 skip 記録を挿入する。`runPostJob(slot, env)` 呼び出しは `runPostJob(slot, env, ctx, runId)` に変える（Task A9）。
...
export async function latestTraceByStage(): Promise<Record<string, { status: string; outcome?: string }>> {
  const sb = serverSupabase();
  const { data } = await sb.from("run_trace")
    .select("stage_id,status,outcome,started_at")
    .order("started_at", { ascending: false }).limit(200);
  const out: Record<string, { status: string; outcome?: string }> = {};
  for (const r of data ?? []) if (!out[r.stage_id]) out[r.stage_id] = { status: r.status, outcome: r.outcome ?? undefined };
  return out;
}
...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  return NextResponse.json(await recentTracesForStage(params.id, 20));
}
```
...
export default async function RunDetail({ params }: { params: { id: string } }) {
  const { run, traces } = await runTimeline(params.id);
  return (
    <main className="p-4">
